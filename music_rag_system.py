...
class MusicRAGSystem:
    ...
    def setup_vector_store(self, tracks_df):
        ...
        for _, row in tracks_df.iterrows():
            ...
            if 'popularity' in row and pd.notna(row['popularity']):
                metadata['popularity'] = str(float(row['popularity']))
    ...
    def search_similar_tracks(self, query: str, n_results: int = 10, filter_metadata: Optional[Dict] = None) -> Any:
        """
    ...
    def get_recommendations(self, 
                          user_listening_history: Optional[List[str]] = None, 
                          preference_text: str = "", 
                          n_recommendations: int = 10,
                          genre_filter: Optional[str] = None) -> Tuple[List[str], Any]:
        """
    ...
    def _create_history_summary(self, history_tracks: pd.DataFrame) -> str:
        ...
        if available_features:
            ...
            for feature in available_features:
                value = avg_features if isinstance(avg_features, (float, int)) else avg_features[feature]
